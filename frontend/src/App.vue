...
import {
  Activity, Bell, Bot, BrainCircuit, History, LoaderCircle,
  Mic, MicOff, PlugZap, Radar, RefreshCw, Send,
  Trash2, Volume2, VolumeX, Wifi, WifiOff, X,
...
// ── Historial paginado (real, desde SQLite vía /memory) ──
const historyOffset = ref(0);
const historyTotal  = ref(0);
const loadingMore   = ref(false);
const historyReady  = ref(false);

// ── Alertas persistentes de recordatorios ──
...
const hasMoreHistory  = computed(() => historyOffset.value < historyTotal.value);
...
// ── Historial real y paginado desde SQLite (/memory) ──
// offset=0 trae los mensajes mas recientes. Subir el offset trae
// tandas mas antiguas ("cargar historial anterior").
async function loadHistoryPage(initial = false) {
  if (loadingMore.value) return;
  loadingMore.value = true;
  try {
    const res  = await fetch(`${API_BASE}/memory?limit=${PAGE_SIZE}&offset=${historyOffset.value}`);
    if (!res.ok) throw new Error("memory fetch failed");
    const data = await res.json();
    historyTotal.value = data.total || 0;

    const page = (data.conversation || []).map(i => ({
      id: crypto.randomUUID(),
      role: i.role,
      content: i.content,
      time: i.timestamp || new Date().toISOString(),
    }));

    if (initial) {
      messages.value = page.length ? page : loadLocalFallback();
    } else {
    ...
    if (initial) messages.value = loadLocalFallback();
  } finally {
...
// ── Recordatorios: alerta persistente además de la burbuja en chat ──
...
<template>
  <main class="shell">
    ...
    <!-- BANNER PERSISTENTE DE ALERTAS/RECORDATORIOS -->
    <div v-if="activeAlerts.length" class="alerts-stack">
      <article v-for="alert in activeAlerts" :key="alert.id" class="alert-banner">
        <Bell :size="13" />
    ...
    <section class="workspace">
      ...
      <div v-if="panel === 'chat'" ref="chatScroll" class="chat-window">
        <button v-if="hasMoreHistory" class="load-more-btn" :disabled="loadingMore" @click="loadHistoryPage(false)">
          <LoaderCircle v-if="loadingMore" :size="12" class="spin" />
          <span>{{ loadingMore ? "Cargando..." : "Cargar historial anterior" }}</span>
        </button>
